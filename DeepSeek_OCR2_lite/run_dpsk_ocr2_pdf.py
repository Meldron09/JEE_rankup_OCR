...
# Allow GPU device to be configured via environment variable (default: "0")
...
def get_llm():
    """
    Lazy initialization of vLLM LLM instance.
    Only loads model into GPU when first called.
    """
    global _llm, _sampling_params
    if _llm is None:
        print(f"{Colors.YELLOW}Loading OCR model into GPU...{Colors.RESET}")
        _llm = LLM(
                model=MODEL_PATH,
        ...
        print(f"{Colors.GREEN}OCR model loaded{Colors.RESET}")
                ...
                for points in points_list:
                    ...
                    if label_type == 'image':
                        try:
                            ...
                            cropped.save(f"{output_dir}/images/{jdx}_{img_idx}.jpg")
                        except Exception as e:
                            print(e)
                            pass
                        img_idx += 1
...
def run_ocr_pipeline(input_pdf: str, output_dir: str):
    """
    Runs DeepSeek OCR pipeline on a PDF.
    ...
    # Get LLM instance (lazy load - only loads GPU when needed)
    llm_instance, sampling_params = get_llm()

    # Run inference
    outputs_list = llm_instance.generate(
        batch_inputs,
    ...
    mmd_det_path = os.path.join(output_dir, pdf_name.replace(".pdf", "_det.mmd"))
    mmd_path = os.path.join(output_dir, pdf_name.replace(".pdf", ".mmd"))
    pdf_out_path = os.path.join(output_dir, pdf_name.replace(".pdf", "_layouts.pdf"))
    ...
    print(f"{Colors.GREEN}OCR Completed!{Colors.RESET}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run OCR on PDF")
    parser.add_argument("input_pdf", help="Path to input PDF")
    parser.add_argument("output_dir", help="Output directory")
    args = parser.parse_args()

    run_ocr_pipeline(args.input_pdf, args.output_dir)